...
const providers = [
  ...
  { key: "MICROSOFT", label: "Microsoft", icon: "/img/icons/microsoft.svg", className: "bg-[#2F2F2F] text-white" },
      ...
      {providers.map((provider) => {
        ...
        return (
          <div key={provider.key} className="relative min-h-16">
            ...
            >
              <img src={provider.icon} alt={provider.label} className="h-6 w-6" />
            ...
            {connected && (
              <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center rounded-lg bg-white/70" aria-label={`${provider.label} conectado`}>
